"""
Model training — XGBoost classifier with 5-fold CV, threshold tuning, joblib persistence.
"""
import json
import logging
import warnings
...
def train(X: pd.DataFrame, y: pd.Series, config: dict) -> Tuple[Pipeline, dict, pd.DataFrame, pd.Series]:
    ...
    cv = StratifiedKFold(
        n_splits=config["model"]["cv_folds"], shuffle=True, random_state=random_state
    )
    cv_scores = cross_val_score(
        pipeline, X_train, y_train, cv=cv, scoring=config["model"]["scoring"]
    )
